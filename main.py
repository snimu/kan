...
def change_model_scale(
    ...
    net = make_net(dict(depth=depth, width=width, linear_value=False, num_heads=num_heads, kan_fourier=False))  # kan_fourier: measure equivalent parameters
...
class LatentAttentionBlockWithFourierKAN(nn.Module):
    """ Efficient fused latent-space attention block. Linear keys and queries, nonlinear values."""
    def __init__(self, num_dim, linear_value: bool, num_heads: int, gridsize: int = 300):
        super().__init__()
        # Layer dim parameters. Play around with these, there's likely some undiscovered stuff still!
        self.dim        = num_dim
        self.qk_dim     = self.dim//hyp['net']['qk_dim_div']
...
def make_attn(settings: dict[str, Any]):
    ...
    if settings['kan_fourier']:
        return LatentAttentionBlockWithFourierKAN(settings['width'], settings['linear_value'], settings['num_heads'], settings['gridsize'])
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--kan_fourier",
        type=int, default=0, nargs="+",
        help="If 1, use the Fourier KAN attention, else use the standard KAN attention. "
        "If you provide several values (for example, 0 1 2 3 4), "
        "will be reduced to their booleans without repetition (so False, True). "
        "TYPE: int; DEFAULT: 0"
    )
    parser.add_argument(
        ...
        help="Gridsize for the Fourier KAN attention. "
    ...
    args.gridsize = [args.gridsize] if isinstance(args.gridsize, int) else args.gridsize
    ...
    args.kan_fourier = [args.kan_fourier] if isinstance(args.kan_fourier, int) else args.kan_fourier
    args.kan_fourier = list(set([bool(v) for v in args.kan_fourier]))
...
def get_settings(args: argparse.Namespace) -> list:
    ...
    settings_base =  list(itertools.product(
        args.model_scale, args.depth, args.width, args.num_heads, args.linear_value
    ))
    settings_kan = [(True, gridsize) if kan_fourier else (False, 0) for kan_fourier in args.kan_fourier for gridsize in args.gridsize]
    settings_kan = list(set(settings_kan))  # remove duplicates

    settings = [
        (*setting_base, *setting_kan) 
        for setting_base in settings_base 
        for setting_kan in settings_kan
    ]

    settings = [
        (model_scale, depth, width, num_heads, linear_value, kan_fourier, gridsize) 
        for model_scale, depth, width, num_heads, linear_value, kan_fourier, gridsize in settings 
...
def main():
    ...
    for setting_num, (model_scale, depth, width, num_heads, linear_value, kan_fourier, gridsize) in enumerate(settings):
